...
When the machine starts, the BIOS loads the first 512B into physical RAM address `0x7C00` (sector 1), and then jumps to that memory address, which would look something like  
`PC = 0x7C00 -> first instruction of bootloader (cli)`  
After that, the BIOS releases control to the bootloader, which then loads the kernel to physical address `0x1000` (sector 2 onwards) and does a jump to that address, handing off control to the kernel. (which you get to write!)  
